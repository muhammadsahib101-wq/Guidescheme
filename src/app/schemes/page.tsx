...
function SchemesContent() {
	...
	useEffect(() => {
		...
		return () => controller.abort();
	}, [skip, stateId, categoryId, isSearching]);
	...
	const getPageTitle = () => {
		...
		if (categoryName) {
			return `${categoryName} Schemes | Government Welfare Programs 2025`;
		}
	...
	const structuredData = {
		...
		"@type": "CollectionPage",
		name: getPageTitle(),
		description: `Explore government schemes and welfare programs${
	...
	return (
		<>
			<Head>
				...
				<meta
					name="description"
					content={`Discover government schemes${
						stateName ? ` in ${stateName}` : ""
					}${
						categoryName ? ` for ${categoryName}` : ""
					}, including benefits, eligibility criteria, and application processes.`}
				/>
				<meta
					name="keywords"
					content={`government schemes${stateName ? `, ${stateName}` : ""}${
						categoryName ? `, ${categoryName}` : ""
					}, welfare programs, benefits, eligibility`}
				/>
				...
				<meta
					...
					content={`Explore government schemes${
						stateName ? ` in ${stateName}` : ""
					}${categoryName ? ` for ${categoryName}` : ""}.`}
				/>
							...
							<form onSubmit={handleSearch} className="relative mb-8">
								...
								{isSearching && (
									<button
										onClick={() => {
											setSearchQuery("");
											fetchData(); // Reset to initial state
										}}
